...
def signup(username: str, password: str) -> tuple[str, str]:
    ...
    with _connect() as conn:
        ...
        cur = conn.execute(
            "INSERT INTO hassan_users (username, password_hash, salt, created_at) VALUES (?, ?, ?, ?)",
...
def login(username: str, password: str) -> tuple[str, str]:
    ...
    with _connect() as conn:
        row = conn.execute(
            "SELECT id, username, password_hash, salt FROM hassan_users WHERE username = ? COLLATE NOCASE",
            (u,),
        ).fetchone()
...
def _create_session(user_id: int, token: str) -> None:
    now = _utc_now()
    exp = (now + timedelta(days=SESSION_DAYS)).isoformat()
    with _connect() as conn:
        conn.execute(
            "INSERT INTO hassan_sessions (token, user_id, created_at, expires_at) VALUES (?, ?, ?, ?)",
